...
class sipManage():
    ...
    def add_sip(self,token,username,password,ip,port,privately,lineType,groupSize):
        ...
        data = {"username":username,"password": password,"ip": ip,"port": port,
                "locationArray": ["动态显示"],"status": 0,"privately": privately,"lineType": lineType,"groupSize": groupSize,
    ...
    def update_sip(self,token,group_number,sip_id,username,password,ip,port,privately,lineType,groupSize):
        headers = {'Content-Type': 'application/json', 'token': token}
        data = {"username": username,"password": password,"ip": ip,"port": port,
                "locationArray": ["动态显示","undefined"],"status": 4,"privately": privately,"lineType": lineType,
	            "groupSize": groupSize,"industryTypeName": "在线教育","bindId": privately,"bindName": "648186030@qq.com",
